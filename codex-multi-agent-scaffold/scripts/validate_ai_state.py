...
def validate(target: Path, strict: bool) -> list[Check]:
    ...
    extra_keys = sorted(set(status.keys()) - REQUIRED_STATUS_KEYS)
    if missing_keys:
        add(
            checks,
            "FAIL",
    ...
    check_referenced_file(
        ...
        required_when=task_status in {"done", "review-pass", "review-failed", "need-human-review"},
    )
    check_referenced_file(
        checks,
        target,
...
def detect_state_conflicts(checks: list[Check], status: dict[str, Any]) -> None:
    ...
    if phase == "reviewing" and task_status != "done":
        conflicts.append("reviewing phase should use task_status done")
    ...
    if phase == "completed" and task_status not in {"review-pass", "done"}:
